...
const FUNDS_AVAILABLE = 234_221.12
...
export default function OutboundDisbursementPage() {
  ...
  const fromLoa = searchParams.get('from') === 'loa'
  const initialMethod = (searchParams.get('method') as TransferMethod) || 'ach'
  const outboundPath = fromLoa
    ? `/account/${accountId}/move-money/new-loa/details`
    : `/account/${accountId}/move-money/outbound`
  const instructionsPath = `/account/${accountId}/move-money/outbound/instructions`
  const newWireInstructionsPath = `/account/${accountId}/move-money/new-loa/wire-instructions?flow=same`
  ...
  const [transferMethod, setTransferMethod] = useState<TransferMethod>(initialMethod)
      ...
      <main className="flex flex-1 justify-center overflow-y-auto px-4 py-10">
        <div className="w-full max-w-[700px] space-y-8 pb-10">
          ...
          <section className="space-y-1 px-2">
            ...
            <div className="pt-3">
              <div className="flex h-9 items-center justify-between">
                ...
                <p className="text-sm font-medium text-foreground">{formatCurrency(FUNDS_AVAILABLE * 100)}</p>
              </div>
          ...
          <section className="space-y-4">
            ...
            {tableConfig ? (
              fromLoa && transferMethod === 'wire' ? (
                <div className="flex flex-col items-center gap-4 py-8">
                  <Blocks className="size-6 text-muted-foreground" />
                  <p className="text-sm text-muted-foreground">
                    No wire instructions and/or standing LOAs on file
                  </p>
                  <Button
                    asChild
                    variant="ghost"
                  ...
                  >
                    <Link href={newWireInstructionsPath}>
