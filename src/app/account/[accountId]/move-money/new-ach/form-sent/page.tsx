...
export default function NewAchFormSentPage() {
  const router = useRouter()
  const params = useParams()
  const accountId = params?.accountId as string

  const reviewPath = `/account/${accountId}/move-money/new-ach/review`
  const instructionNotesPath = `/account/${accountId}/move-money/new-ach/instruction-notes`
        ...
        <main className="flex-1 overflow-y-auto py-10">
          <div className="w-full max-w-[700px] pb-10">
            <div className="relative border border-[#faf9f9]">
              <Image
                src="/ach-form-preview.png"
                alt="ACH Authorization Form Preview"
                width={700}
                height={906}
                className="w-full"
                priority
