...
function getActiveStepIndex(pathname: string): number {
  if (pathname.includes('/confirmation')) return OUTBOUND_STEPS.length
  for (let i = OUTBOUND_STEPS.length - 1; i >= 0; i--) {
    if (pathname.includes(`/${OUTBOUND_STEPS[i].segment}`)) return i
  }
  return -1
}

export function HeaderMoneyMovement() {
  const params = useParams()
  const pathname = usePathname()
  const accountId = params?.accountId as string
  const isOutboundFlow = pathname?.includes('/outbound/')
  const closePath = isOutboundFlow
    ? `/account/${accountId}/move-money`
    : accountId
      ? `/account/${accountId}`
      : '/'
  const activeStep = isOutboundFlow ? getActiveStepIndex(pathname) : -1
