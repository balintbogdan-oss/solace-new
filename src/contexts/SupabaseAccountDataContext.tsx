...
export function SupabaseAccountDataProvider({ 
  children, 
  accountId 
}: { 
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (accountData) {
        ...
        // Save empty data to Supabase
        await supabaseAccountService.saveAccountData(emptyAccountData);
      }
    } catch (err) {
  ...
  const saveData = useCallback(async (newData: AccountData) => {
    try {
      ...
      await supabaseAccountService.saveAccountData(newData);
      setData(newData);
      // Update cache
  ...
  // Reset to seed data (not applicable for Supabase)
  ...
  const preloadAccountData = useCallback(async (targetAccountId: string) => {
    ...
    try {
      const accountData = await supabaseAccountService.getAccountData(targetAccountId);
      if (accountData) {
...
export function useAccountData() {
  ...
  if (context === undefined) {
    throw new Error('useAccountData must be used within a SupabaseAccountDataProvider');
  }
  return context;
}
